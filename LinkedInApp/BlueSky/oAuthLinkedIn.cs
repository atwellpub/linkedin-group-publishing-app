...
namespace Bluesky
{
    ...
    public class OAuthLinkedIn : oAuthBase
    {
        ...
        public string _consumerKey = "pp4dq48v84jq";
        public string _consumerSecret = "rmOqZzWR8acDknRh";

        public enum Method { GET, POST, PUT, DELETE };
        public const string USER_AGENT = "LInkedIn Application 1.0 User Agent";
        ...
        public const string CALLBACK ="LinkedinApp";
        ...
        /// </summary>
        /// <returns>The request token.</returns>
        public String authorizeToken(string user,string pass) {
            if (string.IsNullOrEmpty(Token))
            {
                Exception e = new Exception("The request token is not set");
                throw e;
            }
            Browser browser = new Browser(this, user, pass);
            browser.StatBrowserAuth();
            Token = browser.Token;
        ...
        public string APIWebRequest(string method, string url, string postData)
        {
            ...
            webRequest.Headers.Add("Authorization", "OAuth realm=\"https://api.linkedin.com/\",oauth_consumer_key=\"" + this.ConsumerKey + "\",oauth_token=\"" + this.Token + "\",oauth_signature_method=\"HMAC-SHA1\",oauth_signature=\"" + HttpUtility.UrlEncode(sig) + "\",oauth_timestamp=\"" + timeStamp + "\",oauth_nonce=\"" + nonce + "\",oauth_verifier=\"" + this.Verifier + "\", oauth_version=\"1.0\"");            
